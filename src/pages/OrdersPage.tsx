...
export default function OrdersPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Entregado': return 'bg-emerald-100/50 text-emerald-700';
      case 'Pendiente': return 'bg-amber-100/50 text-amber-700';
      case 'En Ruta': return 'bg-blue-100/50 text-blue-700';
      case 'Cancelado': return 'bg-rose-100/50 text-rose-700';
      case 'En Preparación': return 'bg-indigo-100/50 text-indigo-700';
  ...
  return (
    <div className="space-y-8">
      ...
      <FadeIn className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <AnimatedButton className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-bold hover:bg-slate-50 transition-colors shadow-sm">
        ...
        <GlassCard className="flex flex-wrap items-center gap-3 p-3">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg border-slate-200 text-sm focus:ring-[#1a3b5b] focus:border-[#1a3b5b] bg-white/50"
              value={searchTerm}
          ...
          <div className="ml-auto border-l border-slate-200 pl-3 flex items-center gap-2">
            <AnimatedButton className="p-2 text-slate-500 hover:text-[#1a3b5b] transition-colors">
                ...
                <AnimatePresence mode="popLayout">
                  {paginatedOrders.map((order) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest backdrop-blur-sm ${getStatusColor(order.status)}`}>
                          {order.status}
                        </span>
                      </td>
                      <td className="p-4 text-right">
                        <div className="flex justify-end gap-1">
                          <AnimatedButton className="p-2 text-slate-400 hover:text-[#1a3b5b] transition-colors">
                          ...
                          <AnimatedButton className="p-2 text-slate-400 hover:text-rose-500 transition-colors">
          ...
          <div className="px-4 py-4 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <AnimatedButton 
                ...
                className="p-2 rounded-lg border border-slate-200 hover:bg-white disabled:opacity-50 transition-colors"
              >
